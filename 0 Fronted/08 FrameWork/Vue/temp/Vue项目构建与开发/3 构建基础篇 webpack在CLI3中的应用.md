...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/7/22/164c05d8ad965059~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
> 这里有个问题：
>
> 当我们设置了`publicPath: /a/`之后，开发环境的访问地址和本地生产环境的访问地址都已经发生了变化。
>
> 开发环境： `localhost:8080/a`
>
> 生产环境： `http://127.0.0.1:8080/a/`  
>
> 还要主要开发时的dist中的路径， 一般是新设置了文件夹a， 将dist下生成的内容放到a文件夹下，在dist文件下启动本地服务器（http-server, 或， server）。
>
> 
...
开启 source map 后，我们打包输出的文件中会包含 js 对应的 .map 文件，其用途可以参考：[JavaScript Source Map 详解](https://link.juejin.cn/?target=http%3A%2F%2Fwww.ruanyifeng.com%2Fblog%2F2013%2F01%2Fjavascript_source_map.html)

### d. chainWebpack

chainWebpack 配置项允许我们更细粒度的控制 webpack 的内部配置，其集成的是 [webpack-chain](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fmozilla-neutrino%2Fwebpack-chain) 这一插件，该插件可以让我们能够使用链式操作来修改配置，比如：
...
关于 webpack-chain 的使用可以参考其 github 官方地址：[github.com/mozilla-neu…](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fmozilla-neutrino%2Fwebpack-chain)，它提供了操作类似 JavaScript Set 和 Map 的方式，以及一系列速记方法。

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/10/30/166c58e690ddb43d~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
当然除了以上参数，其支持所有的 webpack-dev-server 中的选项，比如 `historyApiFallback` 用于重写路由（会在后续的多页应用配置中讲解）、progress 将运行进度输出到控制台等，具体可参考：[devServer](https://link.juejin.cn/?target=https%3A%2F%2Fwww.webpackjs.com%2Fconfiguration%2Fdev-server%2F)

以上讲解了 vue.config.js 中一些常用的配置项功能，具体的配置实现需要结合实际项目进行，完整的配置项可以查看：[vue.config.js](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fvuejs%2Fvue-cli%2Fblob%2Fce3e2d475d63895cbb40f62425bb6b3237469bcd%2Fdocs%2Fzh%2Fconfig%2FREADME.md)
...
如图所示： ![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/7/22/164c15d1f97432b5~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
我们可以看到每个插件上方都添加了使用 chainWebpack 访问的方式，同时我也添加了每个插件相应的用途注释，需要注意的是要区分 webpack 内置插件和第三方插件的区别，如果是内置插件则无需安装下载，而外部插件大家可以直接访问：[www.npmjs.com/](https://link.juejin.cn/?target=https%3A%2F%2Fwww.npmjs.com%2F) 搜索对应的插件，了解其详细的 api 设置。
...
· webpack-merge合并原理基于Object.assign()实现的属性覆盖
· chainwebpack调用模板：config.module.rule(test-value).oneOf(...).use(loader-name).tap((options)=>{
merge(options,{...})
})