...
- 通过 npm run serve -- --模块名 启动指定的模块，如果没有指定 取 pages.json 里的第一项
- eg: npm run serve -- --handleThingWithoutCard
...
- 通过 `npm run serve -- --[模块名]` 启动本地 , 执行 `development`
- 通过 `npm run testing -- --[模块名]` 打包测试 , 执行 `testing`
- 通过 `npm run stage -- --[模块名]` 打包预生产 , 执行 `staging`
- 通过 `npm run build -- --[模块名]` 打包正式 , 执行 `production`
...
这里我们并没有定义很多变量，只定义了基础的 VUE_APP_ENV `development` `testing` `staging` `production`  
...
// 本地环境配置
module.exports = {
  title: 'vue-h5-template-ZheJiangPersonal',
...
module.exports = {
  plugins: {
    ...
    'postcss-pxtorem': {
      rootValue: 37.5,
      propList: ['*']
      // selectorBlackList: ['van-'] 是否对vant 组件也进行适配  建议不适配
    }
  }
}
```
...
vue-h5-template-ZheJiangPersonal 所有全局样式都在 `$root/src/assets/css` 目录下设置
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      doDispatch() {
        this.$store.dispatch('setUserName', 'xxxxxx')
      }
    }
  }
</script>
...
### <span id="axios">✅ Axios 封装及接口管理</span> （项目中暂时未用到）
...
pages 文件夹下 每个文件夹属于一个单独的模块,项目启动单位为单一模块,在根目录下 pages.json 配置， 启动或打包项目时务必指定
你当前所开发的模块名称。

# 关于 pages.json

1. 接受一个数组，每一项都是字符串，来指定有哪些模块。每一项代表对应模块【模块名+开发者+菜单号】信息。
2. 模块的增/减, 都要对 pages.json 数组进行修改
