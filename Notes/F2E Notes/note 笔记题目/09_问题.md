...
- 排列方式：
    ...
    - 块元素：独占一行，从上到下垂直排列。
- 尺寸设置：
    - 行内元素：默认宽度为其内容宽度，无法设置 width 和 height。
    - 块元素：默认宽度为父容器的 100%，可以设置 width 和 height。
- 内外边距（margin/padding）：
    - 行内元素：只能设置左右 margin 和 padding，上下无效。
    - 块元素：可以设置所有方向的 margin 和 padding。
- 包含规则：
...
* 避免使用标签和通用选择器作为关键选择器
* 类命名在需要时体现层级关系
* 搞清楚哪些会触发重新布局（reflow）、重绘（repaint）和合成（compositing）
...
function decorate(component) {
  component.newFunction = function() {
    // 添加新功能
  };
  return component;
}
...
// 使用装饰器
const getUserInfoWithLogging = withLogging(getUserInfo);

// 测试
console.log(getUserInfoWithLogging(123));
...
// 添加订阅者
publisher.subscribe(data => console.log('订阅者1收到:', data));
publisher.subscribe(data => console.log('订阅者2收到:', data));
...
#### 事件循环
**是什么?**
事件循环是一种实现单线程非阻塞的执行机制，通过协调执行栈和事件队列来处理同步和异步操作。在JavaScript的单线程环境下实现异步操作和并发处理，避免长时间的同步操作阻塞程序执行。
...
**具体流程**
> [personal-blog/深入理解JavaScript系列/深入理解 JavaScript 之事件循环(Event Loop).md at master · Jacky-Summer/personal-blog (github.com)](https://github.com/Jacky-Summer/personal-blog/blob/master/%E6%B7%B1%E5%85%A5%E7%90%86%E8%A7%A3JavaScript%E7%B3%BB%E5%88%97/%E6%B7%B1%E5%85%A5%E7%90%86%E8%A7%A3%20JavaScript%20%E4%B9%8B%E4%BA%8B%E4%BB%B6%E5%BE%AA%E7%8E%AF(Event%20Loop).md)
...
#### 执行栈
**是什么**
执行栈是一种后进先出的数据结构，用于存储和跟踪JavaScript代码的执行顺序。

**为什么**
为了追踪和管理函数调用的执行上下文，确保代码能够按照正确的顺序执行和返回。

**具体流程**
当调用一个函数时，该函数被压入栈顶；当函数执行完毕，该函数从栈顶弹出，继续执行下一个函数。


#### 事件队列
**是什么**
事件队列是一种先进先出的数据结构，用于存储异步操作的回调函数，分为宏任务队列和微任务队列。

**为什么**
为了实现异步操作，将耗时的任务延后执行，避免阻塞主线程，提高程序的响应性。

**具体流程**
当异步操作完成时，其回调函数被添加到相应的事件队列；当执行栈为空时，事件循环会按照先微任务后宏任务的顺序，从队列中取出回调函数执行。
...
* 全局路由守卫每次判断用户是否已登录
	* 没有登录,则跳到登录页
	* 已经登录,则判断当前要跳转的路由,用户是否有权限访问
		* 没有权限访问,则跳到事先定义好的界面(403,404之类)
...
##### 方案2-动态路由
**描述**
初始化时,先加载不需要权限控制的路由,例如登录页,404等错误页.登陆后,获取用户权限信息,然后筛选用户有权访问的路由,在全局守卫里调用`addRoutes`添加到路由中
...
添加错误处理和加载状态: 
**错误处理和加载状态**：除了基本用法，你还可以为异步组件添加错误处理和加载状态。例如：
```js
...
**提升用户体验**：合理使用异步组件可以优化页面性能，但是滥用会导致过多的网络请求，反而降低性能。建议仅对体积较大、初始不需要展示的组件进行异步加载。
...
选项式api:
- 选项式API，通过定义`data、computed、watch、method`等属性与方法，共同处理页面逻辑；
- 缺点：
...
- 优点：
    ...
    - 逻辑复用：
        - 在Vue2中，当混入多个`mixin`会存在两个非常明显的问题：命名冲突，数据来源不清晰；
...
### 列表分页频繁点击的竞态问题
#### 来源
> [如何解决前端常见的竞态问题前端常见的搜索、分页、选项卡等切换场景中，由于网络不确定性，先发出的请求不一定先响应，这会造成 - 掘金 (juejin.cn)](https://juejin.cn/post/7128205011019890695)
...
onCanceled = cancel => {
	...
	reject(!cancel || cancel.type ? new CancelError(null,config.request) : cancel);
	request.abort();
	request = null;
}
```
...
cancelToken 从 `v0.22.0` 开始已被 axios 弃用。原因是基于实现该 API 的提案 [cancelable promises proposal](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Ftc39%2Fproposal-cancelable-promises "https://github.com/tc39/proposal-cancelable-promises") 已被撤销。
...
我们以`awesome-imperative-promise` 为例，来看看 cancel 的实现，它的 cancel 实现基于**指令式 promise(可实现直接能调用resolve, reject, cancel等方法)，** 源码一共只有 40 行。
