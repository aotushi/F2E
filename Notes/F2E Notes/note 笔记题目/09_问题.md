...
#### BFC应用
清除浮动: BFC可以包含浮动,用来清除浮动防止父元素高度塌陷
防止margin重叠: 创建新的BFC来防止相邻元素间的margin重叠
自适应两栏布局: 利用BFC不与浮动元素重叠的特性
防止文字环绕: 为父元素设置overflow:hidden使其成为BFC
...
- 使用`ref`声明的数据，在JS中使用的时候，需要添加`.value`的后缀；
...
![fc95bf60-3ac6-11eb-ab90-d9ae814b240d](https://jsd.cdn.zzko.cn/gh/aotushi/image-hosting@master/documentation/fc95bf60-3ac6-11eb-ab90-d9ae814b240d.3qhhp95pi200.webp)
...
![](https://jsd.cdn.zzko.cn/gh/aotushi/image-hosting@master/documentation/.3gam4th7xli0.webp)


### Vue实例挂载的过程中发生了什么?
> [面试官：Vue实例挂载的过程 | web前端面试 - 面试官系列 (vue3js.cn)](https://vue3js.cn/interview/vue/new_vue.html#%E4%B8%80%E3%80%81%E6%80%9D%E8%80%83)
...
### Vue常见的修饰符有哪些？
> [面试官：Vue常用的修饰符有哪些有什么应用场景 | web前端面试 - 面试官系列 (vue3js.cn)](https://vue3js.cn/interview/vue/modifier.html#%E4%B8%80%E3%80%81%E4%BF%AE%E9%A5%B0%E7%AC%A6%E6%98%AF%E4%BB%80%E4%B9%88)
...
#### 表单修饰符
表单用的最多的input标签，指令多是v-model，关于v-model修饰符有：
...
* native  在移动端，当我们在监听元素滚动事件的时候，会一直触发`onscroll`事件会让我们的网页变卡，因此我们使用这个修饰符的时候，相当于给`onscroll`事件整了一个`.lazy`修饰符
...
- prop 
	...
	- 用途: 通过自定义属性存储变量，避免暴露数据; 防止污染 HTML 结构
- camel  允许在使用 DOM 模板时将 `v-bind` property 名称驼峰化

使用案例
```html
<input id="uid" title="title1" vaule="1" :index.prop="index" >
```
...
### Vue中的移动端适配
```js
安装postcss-loader postcss-px2rem
```
### 移动端问题
