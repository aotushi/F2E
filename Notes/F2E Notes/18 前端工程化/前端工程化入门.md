...
#### 1.安装vscode插件Prettier
#### 2.项目根目录下新建`.prettierrc.js`
```js
module.exports = {
...
> 配置前两步后，虽然已经配置**prettier**格式化规则，但还需要让**vscode**来支持保存后触发格式化; 在项目根目录新建 **.vscode**文件夹，内部新建**settings.json**文件配置文件
...
**2.初始化lint文件,也就是在根目录下创建一个`.eslintc.js`或`.eslintrc.*`**
...
* To check syntax, find problems, and enforce code style 
  * Airbnb、Standard、Google(只有选择最后一个选项, 后续会出现3种JS代码规范的选择)
...
生成的文件以cjs结尾: 为什么生成的配置文件名称是`.eslintrc.cjs`而不是`.eslintrc.js`？
