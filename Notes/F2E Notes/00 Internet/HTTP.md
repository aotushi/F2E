...
强缓存主要是由 `Cache-control` 和 `Expires` 两个响应头决定的. `Cache-control` 优先级比 `Expires` 优先级高


#### 判断条件
`Expires` 的值和Header中的 `Date` 属性的值来判断是否缓存还有效。

#### Expires概述

* `Expires` 是 `Web` 服务器**响应消息**头字段，在响应 `http` 请求时告诉浏览器在过期时间前浏览器可以直接从浏览器缓存取数据，而无需再次请求。
...
| 名称               | 作用                           | 案例                              | 其它  |
| ---------------- | ---------------------------- | ------------------------------- | --- |
| max-age=31536000 | 声明缓存有效期(距离请求发起的时间的秒数之内使用强缓存) | Cache-Control: max-age=31536000 |     |
| no-store         | 不使用缓存                        | Cache-Control: no-store         |     |
| no-cache         | 有缓存但需要重新验证                   | Cache-Control: no-cache         |     |
