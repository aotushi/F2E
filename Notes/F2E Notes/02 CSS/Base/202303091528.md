...
* 当Flex项目显式设置width(一个具体值,不为auto),为显示设置flex-basis时,浏览器会采取Flex项目上的`width`属性的值
* 如果Flex项目同时都未显式指定`flex-basis`和`width`属性的值，那么`flex-basis`将回退到基于Flex项目的内容计算宽度.
* 如果Flex项目同时显式设置width与flex-basis(非auto默认值),flex-basis属性值将会替代width值.
	* flex-basis小于width,且width小于flex项目内容最小值(min-content), flex-basis值等于width值
	* flex-basis小于width,且width大于flex项目内容最小值(min-content), flex-basis值等于flex项目内容最小值
	* flex-basis大于width,但小于flex项目的内容最小值,flex-basis会等于width值
	* flex-basis大于width,且同时大于flex项目内容最小值,flex-basis不会被任何其他值替代.
...
**当**
* 当Flex项目的Flex-basis和width取初始值auto时,Flex项目的初始化尺寸是fit-content;
* 但当flex-shrink显式重置为0时,Flex项目不可收缩,Flex项目的初始化尺寸是max-content
* 当Flex项目显式设置width,flex-basis未设置为默认值时,浏览器会采用Flex项目上的width属性的值
* 

| 条件: flex-grow | 条件2: flex-shrink | flex-basis   | width        | flex项目 |
| --------------- | ------------------ | ------------ | ------------ | -------- |
| 默认值          | 默认值             | 没有显式赋值 | 没有显式赋值 |          |
