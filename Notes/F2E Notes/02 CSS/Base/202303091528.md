...
#### 作用
<<<<<<< HEAD
>flex-basis 属性在 Flexbox 中对 Flex 项目尺寸起着决定性的作用。简单地说，<span style="color:red">flex-basis 属性在任何空间分配发生之前，会对 Flex 项目的尺寸大小进行初始化</span> 。即，在任何 Flex 容器空间（剩余空间或不足空间）分配发生之前初始化 Flex 项目尺寸 。

延伸:
flex容器空间是由 flex-grow 、flex-shrink 和 Flex 容器的对齐方式来分配的。
=======
>flex-basis 属性在任何空间分配发生之前，会对 Flex 项目的尺寸大小进行初始化 。即，在任何 Flex 容器空间（剩余空间或不足空间）分配发生之前初始化 Flex 项目尺寸 。
>>>>>>> 99d3255 (docs)
...
#### 可用于flex-basis的值
* **width:**  缺点是固定值可能出现的内容断行/溢出.
* **min-width:** 为了避免设置width属性固定值可能出现的内容断行或内容溢出容器,建议使用min-width代替width(可以防止width值小于min-width指定的值所造成的布局缺陷, < min-width默认值auto,浏览器计算出来的值是`0` >.
* **max-width:** 在容器（或元素）上显式设置 `max-width` 属性值时，可以防止 `width` 属性值超过了 `max-width` 指定的值，造成内容溢出。
* **数学表达式**: 如 `calc()` 、`min()` 、`max()` 和 `clamp()` 等比较函数
