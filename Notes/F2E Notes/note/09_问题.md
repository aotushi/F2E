...
* defer则是JS加载完成后，在文档解析完成后再执行JS脚本
...
**CSS3新增的结构化伪类**
:nth-child(n)：选择父元素下的第n个子元素。n可以接受具体的数值，也可以接受函数（如4n-1）；可以接受odd(奇数)、even(偶数)
:nth-last-child(n)
:last-child：最后一个元素
:only-child：父元素的唯一元素
:nth-of-type(n)：父元素下第n个elem元素，n接受的格式和nth-child一样。
与nth-child的区别：elem:nth-of-type(n)是“选择父元素下第n个elem元素”；elem:nth-child(n)是“这个伪类选中父元素下的第n个子元素，并且这个子元素的标签名为elem”。

elem:empty选择不包含子元素和内容的elem标签。
elem:enabled和elem:disabled可用、禁用状态
elem:checked匹配每个选中的input元素（适用于单选框和复选框）
...
Vue采用**数据劫持**结合**发布者-订阅者模式**的方法，通过Object.defineProperty()来劫持各个属性的setter,getter属性，在数据变动时，发布消息通知订阅者，触发更新回调函数，重新渲染视图.
...
### Vue 列表为什么加 key？
* 虚拟DOM中，当更新子节点时，会查找新旧虚拟节点列表中相同的节点，从而进行更新。key是给每个虚拟节点的唯一id，依靠key，diff操作会更准确、快速。
* 注意不能使用数组的index作为key值，因为新增数组元素和删除数组元素时，每个元素的index可能会发生改变，从而无法达到想要的效果。
* 如果没有key属性，Vue会复用元素，只改变元素的内容，而不会有新的元素被添加进来，也不会有旧的元素被删除。
...
##### hash模式
原理：使用HTML的锚点技术，实现路由匹配和跳转；通过location.hash获取url中的锚点值，完成 url 跳转，且不会重新加载页面，即不刷新页面。
案例：url(`http://www.abc.com/#/hello`)，其中#/hello是hash的值，特点在于锚点值虽然出现在url中，但不会被包括在HTTP请求中。对后端没有影响，因此改变hash不会重新加载页面。
缺点：地址中会出现#号，比较丑陋

##### history模式
原理：利用了 HTML5 History Interface 中新增的 pushState() 和 replaceState() 方法（需要特定浏览器支持）。使用history.pushState完成url的跳转，且不会重新加载页面。

缺点：在生产环境下，如果用户在浏览器中直接访问url路径，则会报错 Not Found 。所以在生产环境下，需要后台服务器的配置支持才行（在开发环境下是没问题的）

**pushState()相比较hash的优点：**
pushState()设置的新URL可以是与当前 URL 同源的任意 URL；而 hash 只可修改`#`后面的部分，因此只能设置与当前URL同文档的 URL；
pushState()设置的新URL可以与当前 URL 一模一样，这样也会把记录添加到栈中；而hash设置的新值必须与原来不一样才会触发动作将记录添加到栈中；
hash模式下，仅hash符号之前的内容会被包含在请求中，如 `http://www.abc.com`，因此对于后端来说，即使没有做到对路由的全覆盖，也不会返回 404 错误。
history模式下，前端的URL必须和实际向后端发起请求的 URL 一致，如 `http://www.abc.com/book/id`。
...
#### 路由模式区别
1.URL格式
* 哈希模式：使用#作为分隔符，后面跟着路由的路径。例如：http://example.com/#/path/to/route
* 历史模式：使用正常的URL格式，不使用#符号。例如：http://example.com/path/to/route
2.历史记录
* 哈希模式：只有#符号前面的URL会被浏览器记录在历史记录中。
* 历史模式：所有的URL都会出现在浏览器的历史记录中。
3.服务器配置
* 哈希模式：不需要特殊的服务器配置，因为所有的路由都是在客户端处理的。
* 历史模式：需要服务器的支持，因为首次请求服务器上不存在的URL时，需要返回index.html文件以便客户端进行前端路由。
