...
#### 是什么
BFC(Block formatting context)直译为"块级格式化上下文".它是块级盒子出现或与其它元素浮动交互的区域.
...
最常见的Formatting context有Block fomatting context (简称BFC)和 Inline formatting context (简称IFC).
CSS2.1 中只有 BFC和 IFC, CSS3 中还增加了 GFC和 FFC。
...
### relative,fixed,absolute,static定位 //?
...
### display:none,visibile:hidden,opacity:0  //?
